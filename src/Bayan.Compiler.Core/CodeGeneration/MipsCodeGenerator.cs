...
public sealed class MipsCodeGenerator
{
    ...
    private static void EmitInstruction(StringBuilder builder, IrInstruction instruction)
    {
        ...
        // يحول العلامة إلى تسمية MIPS منتهية بنقطتين، ويضيف إطار المكدس إن كانت بداية إجراء.
        if (instruction.Opcode == IrOpcode.Label)
        {
            builder.AppendLine(instruction.TargetLabel + ":");
            if (instruction.TargetLabel != null && instruction.TargetLabel.StartsWith("proc_"))
            {
                builder.AppendLine("    addi $sp, $sp, -8");
                builder.AppendLine("    sw $ra, 4($sp)");
                builder.AppendLine("    sw $fp, 0($sp)");
                builder.AppendLine("    move $fp, $sp");
            }
            return;
        }
        ...
        {
            builder.AppendLine("    move $sp, $fp");
            builder.AppendLine("    lw $fp, 0($sp)");
            builder.AppendLine("    lw $ra, 4($sp)");
            builder.AppendLine("    addi $sp, $sp, 8");
