...
public sealed class Lexer
{
    ...
    */
    ...
    /// <summary>
    /// يقرأ ثابتاً نصياً بين علامتي اقتباس مزدوجتين.
    /// </summary>
    private void ScanString(int start, int startLine, int startColumn)
    {
        // يقرأ حتى علامة الاقتباس الختامية أو حتى نهاية الملف.
        while (!IsAtEnd && Peek() != '"')
        {
            Advance();
        }

        // يبلغ عن سلسلة غير مغلقة عند الوصول إلى نهاية الملف.
        if (IsAtEnd)
        {
        ...
        // يستهلك علامة الاقتباس الختامية.
        Advance();

        // يستخرج محتوى النص من دون علامتي الاقتباس لتستخدمه مرحلة التنفيذ لاحقاً.
        var value = _source.Substring(start + 1, _position - start - 2);
        var span = CreateSpan(start, _position - start, startLine, startColumn);

        // يضيف ثابت النص إلى قائمة Tokens الناتجة.
        _tokens.Add(new Token(TokenType.String, value, span));
    }

    /// <summary>
    /// Scans a single character literal delimited by an ASCII or Arabic single quote.
    /// </summary>
    private void ScanCharacter(int start, int startLine, int startColumn, char delimiter)
    {
        // Reject an empty literal, a newline, or EOF before reading a character value.
        if (IsAtEnd || Peek() == '\n' || Peek() == delimiter)
        {
        ...
        // Consume exactly one UTF-16 character as required by the current language specification.
        char value = Advance();
        ...
        // Consume the closing delimiter and emit the literal content without its quotes.
        Advance();
